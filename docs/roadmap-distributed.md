...
| peer.list 工具 | done — tape tool，LLM 可查询在线 peer |
| skill 同步 | done — peer.skills + peer.pull_skill，请求/响应协议 |
...
- [x] **skill 同步** — peer.skills + peer.pull_skill，LLM 驱动拉取和采纳
- [ ] **任务委派** — 一个 cub 把子任务发给更适合的 peer
- [x] **peer.list 工具** — 注册为 tape tool，LLM 可查询在线 peer
...
| 4 | `peer.list` | 发现和列出网络中的其他 cub | done |
...
```
Phase 1: 单体完善          ✓ done
Phase 2: 简单协作          ◐ P2P + HMAC + skill 同步 done, 任务委派 todo
Phase 3: DHT 集群          ○ 未来，>50 节点时再做
Phase 4: 浏览器运行        ○ 等 moonbitlang/async wasm 支持
Phase 5: 集群治理          ○ 7 个原语，已完成 4 个
```
