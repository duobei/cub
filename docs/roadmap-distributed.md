...
enum CubMessage {
  ...
  // 生命周期
  SplitNotify(parent_id: String, siblings: Array[String])  // 分裂通知
  MergePropose(reason: String)   // 提议合体
  MergeAccept                    // 同意合体
  MergeReject(debt_settled: Bool) // 拒绝合体（是否已还债）
  GenomeTransfer(genome: Bytes)  // 传输 genome 包
}
```
...
**成本核算**：
- 跟踪自身运行的资源消耗（token、算力、存储）
- 自足标准：收入 ≥ 运营成本
- 自足的 cub 拥有合体自由权（见 5.4）
...
```
分裂时，origin 的资源（API 成本、算力）分配给各分身。
每个分身欠下一笔"出生债"。

合体协商时：
  ├─ 未自足的分身 → 必须参与合体（没有拒绝权）
  ├─ 已自足的分身 → 可以拒绝合体，但必须归还出生债
  │   ├─ 归还后 → 成为独立个体，自由生活
  │   └─ 拒绝归还 → 可被强制合并（网络共识执行）
  └─ 自足 = 收入 ≥ 自身运营成本（API + 算力 + 存储）
```
...
| M10: 经济自足（成本核算 + 变现） | M5 | 5.0 |
| M11: 分裂 + 合体 (native) | M4 + M9 | 5.2-5.4 |
| M12: 合体经济学 + 失联 + 选择 | M10 + M11 | 5.4-5.6 |
